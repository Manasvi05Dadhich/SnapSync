...
function AppContent() {
  const { user, loading, login, backendError, retryAuth } = useAuth();
  if (loading) {
    return (
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6 bg-cream px-4">
        ...
        <p className="text-gray-600 text-center">Sign in to get started</p>
        <button
          onClick={login}
          className="px-6 py-3 bg-accent-primary text-white rounded-lg hover:opacity-90 font-medium flex items-center gap-2"
        >
